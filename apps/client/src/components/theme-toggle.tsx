'use client';

import * as React from 'react';
import { useTheme } from 'next-themes';

import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

const iconClassName = 'size-5 shrink-0';

function SunIcon() {
  return (
    <svg className={iconClassName} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path
        d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0 .39-.39.39-1.03 0-1.41l-1.06-1.06zm1.06-10.96c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36c.39.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z"
        fill="currentColor"
      />
    </svg>
  );
}

function MoonIcon() {
  return (
    <svg className={iconClassName} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path
        d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9c0-.46-.04-.92-.1-1.36-.98 1.37-2.58 2.26-4.4 2.26-2.98 0-5.4-2.42-5.4-5.4 0-1.81.89-3.42 2.26-4.4-.44-.06-.9-.1-1.36-.1z"
        fill="currentColor"
      />
    </svg>
  );
}

function getThemeLabels(resolvedTheme?: string) {
  const isDark = resolvedTheme === 'dark';

  return {
    current: isDark ? 'Dark mode' : 'Light mode',
    action: isDark ? 'Switch to light mode' : 'Switch to dark mode',
    next: isDark ? 'light' : 'dark',
  } as const;
}

function ThemeIcon({ resolvedTheme }: { resolvedTheme: string | undefined }) {
  const Icon = resolvedTheme === 'dark' ? MoonIcon : SunIcon;

  return (
    <span
      key={resolvedTheme ?? 'loading'}
      className={cn(
        'inline-flex',
        'motion-safe:animate-in motion-safe:fade-in motion-safe:duration-150'
      )}
    >
      <Icon />
    </span>
  );
}

const toggleButtonClassName =
  'size-11 shrink-0 touch-manipulation motion-safe:active:scale-95';

export function ThemeToggle() {
  const [mounted, setMounted] = React.useState(false);
  const { resolvedTheme, setTheme } = useTheme();

  React.useEffect(() => {
    setMounted(true);
  }, []);

  const labels = getThemeLabels(mounted ? resolvedTheme : undefined);

  function toggleTheme() {
    setTheme(labels.next);
  }

  if (!mounted) {
    return (
      <Button
        type="button"
        variant="ghost"
        className={toggleButtonClassName}
        aria-label="Loading theme preference"
        aria-busy="true"
        disabled
      >
        <SunIcon />
      </Button>
    );
  }

  return (
    <Button
      type="button"
      variant="ghost"
      className={toggleButtonClassName}
      onClick={toggleTheme}
      aria-label={labels.action}
      aria-pressed={resolvedTheme === 'dark'}
      title={labels.current}
    >
      <ThemeIcon resolvedTheme={resolvedTheme} />
    </Button>
  );
}
